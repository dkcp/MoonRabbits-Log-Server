...
app.get('/log/server', (req, res) => {
    ...
    result += '<thead><tr><th style="width: 80px;">날짜</th><th style="width: 80px;">시간</th><th>메시지</th></tr></thead>';
    result += '<tbody>'
...
app.get('/log/packetLog/search', (req, res) => {
    ...
    const endTime = Date.now();
    result = `<h4>${range<60?`${range}분간`:`1시간`} ${packetType} 패킷 로그 조회</h4>`;

    result += '<table border="1" style="width: 100%; border-collapse: collapse;">';
    result += '<thead><tr><th style="width: 80px;">날짜</th><th style="width: 80px;">시간</th><th>메시지</th></tr></thead>';
    result += '<tbody>'
    
    redisClient.zrangebyscore('metricLogs', startTime, endTime, (err, logs) => {
        if(err) {
            console.error('Redis 메트릭 로그 조회 실패:', err);
            return res.status(500).send('메트릭 로그 조회 실패');
        }
